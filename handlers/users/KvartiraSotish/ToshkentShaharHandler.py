...
@dp.callback_query_handler(text="toshshkv", state=None, chat_type="private")
async def first(callback_query: types.CallbackQuery):
    ...
    await ToshkentshHomeSotish.images.set()


@dp.message_handler(is_media_group=True, state=ToshkentshHomeSotish.images, content_types=types.ContentTypes.ANY)
async def starter(message: types.Message, album: List[types.Message], state: FSMContext):
    ...
    await ToshkentshHomeSotish.next()


@dp.message_handler(lambda message: not message.text.replace('.', '').replace(',', '').isdigit(), state=ToshkentshHomeSotish.umumiyMaydon)
async def check_umumiy(message: types.Message):
    await message.reply("❗ Фақат рақамда ёзинг")


@dp.message_handler(state=ToshkentshHomeSotish.umumiyMaydon)
...
@dp.message_handler(lambda message: not message.text.isdigit(), state=ToshkentshHomeSotish.xonalar)
async def check_umumiy(message: types.Message):
    await message.reply("❗ Фақат рақамда ёзинг")


@dp.message_handler(state=ToshkentshHomeSotish.xonalar)
async def umumiymaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "xonalar": text
...
@dp.message_handler(lambda message: not message.text.isdigit(), state=ToshkentshHomeSotish.qavat)
async def check_umumiy(message: types.Message):
    await message.reply("❗ Фақат рақамда ёзинг")


@dp.message_handler(state=ToshkentshHomeSotish.qavat)
async def umumiymaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "qavat": text
...
@dp.message_handler(lambda message: not message.text.isdigit(), state=ToshkentshHomeSotish.qavatlik)
async def check_umumiy(message: types.Message):
    await message.reply("❗ Фақат рақамда ёзинг")


@dp.message_handler(state=ToshkentshHomeSotish.qavatlik)
async def umumiymaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "qavatlik": text
...
# ====================================================================
@dp.callback_query_handler(text='Evroremont', state=ToshkentshHomeSotish.remont, chat_type="private")
...
@dp.callback_query_handler(text="Ta'mirlangan", state=ToshkentshHomeSotish.remont, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = "Таъмирланган"
    await callback_query.answer("Pressed")
    await state.update_data({
...
@dp.callback_query_handler(text="O'rtacha", state=ToshkentshHomeSotish.remont, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = "Ўртача"
    await callback_query.answer("Pressed")
    await state.update_data({
...
@dp.callback_query_handler(text="Ta'mirsiz", state=ToshkentshHomeSotish.remont, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = "Таъмирсиз"
    await callback_query.answer("Pressed")
    await state.update_data({
...
@dp.callback_query_handler(text='Mavjud', state=ToshkentshHomeSotish.jihozlar, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = "бор"
    await callback_query.answer("Pressed")
    await state.update_data({
...
@dp.callback_query_handler(text='Jihozlarsiz', state=ToshkentshHomeSotish.jihozlar, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = "йўқ"
    await callback_query.answer("Pressed")
    await state.update_data({
...
@dp.callback_query_handler(text="bor", state=ToshkentshHomeSotish.gaz)
...
@dp.callback_query_handler(text="yoq", state=ToshkentshHomeSotish.gaz)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext):
    text = "doesnotexist"
    await callback_query.answer("Танланди")
...
@dp.callback_query_handler(text="bor", state=ToshkentshHomeSotish.svet)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext):
    text = "Свет ✔️"
    await callback_query.answer("Танланди")
...
@dp.callback_query_handler(text="yoq", state=ToshkentshHomeSotish.svet)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext):
    text = "doesnotexist"
    await callback_query.answer("Танланди")
...
@dp.callback_query_handler(text="bor", state=ToshkentshHomeSotish.suv)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext):
    text = "Сув ✔️"
...
@dp.callback_query_handler(text="yoq", state=ToshkentshHomeSotish.suv)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext):
    text = "doesnotexist"
    await callback_query.answer("Tanlandi")
...
@dp.message_handler(state=ToshkentshHomeSotish.qoshimchaMalumot)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
...
@dp.callback_query_handler(text='USD', state=ToshkentshHomeSotish.valyuta, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = " $"
    await callback_query.answer("Pressed")
...
@dp.callback_query_handler(text='SUM', state=ToshkentshHomeSotish.valyuta, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = " сўм"
    await callback_query.answer("Pressed")
...
@dp.message_handler(lambda message: not message.text.isdigit(), state=ToshkentshHomeSotish.narxi)
async def check_umumiy(message: types.Message):
    await message.reply("❗ Фақат рақамда ёзинг")


@dp.message_handler(state=ToshkentshHomeSotish.narxi)
...
@dp.message_handler(state=ToshkentshHomeSotish.manzil)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "manzil": text
...
@dp.message_handler(state=ToshkentshHomeSotish.moljal)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "moljal": text
...
@dp.message_handler(state=ToshkentshHomeSotish.telNumberOne)
async def umumiyMaydon(message: types.Message, state: FSMContext):
...
@dp.message_handler(state=ToshkentshHomeSotish.telNumberTwo)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "telNumberTwo": text
...
@dp.message_handler(state=ToshkentshHomeSotish.check)
