...
@dp.callback_query_handler(text="xorazmkv", state=None, chat_type="private")
async def first(callback_query: types.CallbackQuery):
    ...
    await XorazmHomeSotish.images.set()


@dp.message_handler(is_media_group=True, state=XorazmHomeSotish.images, content_types=types.ContentTypes.ANY)
async def starter(message: types.Message, album: List[types.Message], state: FSMContext):
    ...
    await XorazmHomeSotish.next()


@dp.message_handler(lambda message: not message.text.replace('.', '').replace(',', '').isdigit(), state=XorazmHomeSotish.umumiyMaydon)
async def check_umumiy(message: types.Message):
    await message.reply("❗ Фақат рақамда ёзинг")


@dp.message_handler(state=XorazmHomeSotish.umumiyMaydon)
...
@dp.message_handler(lambda message: not message.text.isdigit(), state=XorazmHomeSotish.xonalar)
async def check_umumiy(message: types.Message):
    await message.reply("❗ Фақат рақамда ёзинг")


@dp.message_handler(state=XorazmHomeSotish.xonalar)
async def umumiymaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "xonalar": text
...
@dp.message_handler(lambda message: not message.text.isdigit(), state=XorazmHomeSotish.qavat)
async def check_umumiy(message: types.Message):
    await message.reply("❗ Фақат рақамда ёзинг")


@dp.message_handler(state=XorazmHomeSotish.qavat)
async def umumiymaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "qavat": text
...
@dp.message_handler(lambda message: not message.text.isdigit(), state=XorazmHomeSotish.qavatlik)
async def check_umumiy(message: types.Message):
    await message.reply("❗ Фақат рақамда ёзинг")


@dp.message_handler(state=XorazmHomeSotish.qavatlik)
async def umumiymaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "qavatlik": text
...
# ====================================================================
@dp.callback_query_handler(text='Evroremont', state=XorazmHomeSotish.remont, chat_type="private")
...
@dp.callback_query_handler(text="Ta'mirlangan", state=XorazmHomeSotish.remont, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = "Таъмирланган"
    await callback_query.answer("Pressed")
    await state.update_data({
...
@dp.callback_query_handler(text="O'rtacha", state=XorazmHomeSotish.remont, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = "Ўртача"
    await callback_query.answer("Pressed")
    await state.update_data({
...
@dp.callback_query_handler(text="Ta'mirsiz", state=XorazmHomeSotish.remont, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = "Таъмирсиз"
    await callback_query.answer("Pressed")
    await state.update_data({
...
@dp.callback_query_handler(text='Mavjud', state=XorazmHomeSotish.jihozlar, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = "бор"
    await callback_query.answer("Pressed")
    await state.update_data({
...
@dp.callback_query_handler(text='Jihozlarsiz', state=XorazmHomeSotish.jihozlar, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = "йўқ"
    await callback_query.answer("Pressed")
    await state.update_data({
...
@dp.callback_query_handler(text="bor", state=XorazmHomeSotish.gaz)
...
@dp.callback_query_handler(text="yoq", state=XorazmHomeSotish.gaz)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext):
    text = "doesnotexist"
    await callback_query.answer("Танланди")
...
@dp.callback_query_handler(text="bor", state=XorazmHomeSotish.svet)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext):
    text = "Свет ✔️"
    await callback_query.answer("Танланди")
...
@dp.callback_query_handler(text="yoq", state=XorazmHomeSotish.svet)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext):
    text = "doesnotexist"
    await callback_query.answer("Танланди")
...
@dp.callback_query_handler(text="bor", state=XorazmHomeSotish.suv)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext):
    text = "Сув ✔️"
...
@dp.callback_query_handler(text="yoq", state=XorazmHomeSotish.suv)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext):
    text = "doesnotexist"
    await callback_query.answer("Tanlandi")
...
@dp.message_handler(state=XorazmHomeSotish.qoshimchaMalumot)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
...
@dp.callback_query_handler(text='USD', state=XorazmHomeSotish.valyuta, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = " $"
    await callback_query.answer("Pressed")
...
@dp.callback_query_handler(text='SUM', state=XorazmHomeSotish.valyuta, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = " сўм"
    await callback_query.answer("Pressed")
...
@dp.message_handler(lambda message: not message.text.isdigit(), state=XorazmHomeSotish.narxi)
async def check_umumiy(message: types.Message):
    await message.reply("❗ Фақат рақамда ёзинг")


@dp.message_handler(state=XorazmHomeSotish.narxi)
...
@dp.message_handler(state=XorazmHomeSotish.manzil)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "manzil": text
...
@dp.message_handler(state=XorazmHomeSotish.moljal)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "moljal": text
...
@dp.message_handler(state=XorazmHomeSotish.telNumberOne)
async def umumiyMaydon(message: types.Message, state: FSMContext):
...
@dp.message_handler(state=XorazmHomeSotish.telNumberTwo)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "telNumberTwo": text
...
@dp.message_handler(state=XorazmHomeSotish.check)
