...
async def first(callback_query: types.CallbackQuery):
    ...
    await XorazmHomeSotishHovli.umumiyMaydon.set()


@dp.message_handler(lambda message: not message.text.isdigit(), state=XorazmHomeSotishHovli.umumiyMaydon)
...
@dp.callback_query_handler(text='yoq', state=XorazmHomeSotishHovli.jihozlar, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = "йўқ"
    await callback_query.answer("Pressed")
    await state.update_data({
...
@dp.message_handler(is_media_group=True, state=XorazmHomeSotishHovli.images, content_types=types.ContentTypes.PHOTO)
