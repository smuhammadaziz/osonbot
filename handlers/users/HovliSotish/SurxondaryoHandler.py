...
@dp.callback_query_handler(text="surxonhovli", state=None, chat_type="private")
async def first(callback_query: types.CallbackQuery):
    ...
    await SurxondaryoHomeSotishHovli.images.set()


@dp.message_handler(is_media_group=True, state=SurxondaryoHomeSotishHovli.images, content_types=types.ContentTypes.ANY)
async def starter(message: types.Message, album: List[types.Message], state: FSMContext):
    ...
    await SurxondaryoHomeSotishHovli.next()


@dp.message_handler(lambda message: not message.text.replace('.', '').replace(',', '').isdigit(), state=SurxondaryoHomeSotishHovli.umumiyMaydon)
async def check_umumiy(message: types.Message):
    await message.reply("❗ Фақат рақамда ёзинг")


@dp.message_handler(state=SurxondaryoHomeSotishHovli.umumiyMaydon)
...
@dp.message_handler(lambda message: not message.text.isdigit(), state=SurxondaryoHomeSotishHovli.xonalar)
async def check_umumiy(message: types.Message):
    await message.reply("❗ Фақат рақамда ёзинг")


@dp.message_handler(state=SurxondaryoHomeSotishHovli.xonalar)
async def umumiymaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "xonalar": text
...
# =================================================
@dp.callback_query_handler(text='bor', state=SurxondaryoHomeSotishHovli.oshxona, chat_type="private")
...
@dp.callback_query_handler(text='yoq', state=SurxondaryoHomeSotishHovli.oshxona, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = "йўқ"
    await callback_query.answer("Pressed")
    await state.update_data({
...
@dp.callback_query_handler(text='bor', state=SurxondaryoHomeSotishHovli.hammom, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = "бор"
    await callback_query.answer("Pressed")
    await state.update_data({
...
@dp.callback_query_handler(text='yoq', state=SurxondaryoHomeSotishHovli.hammom, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = "йўқ"
    await callback_query.answer("Pressed")
    await state.update_data({
...
@dp.message_handler(lambda message: not message.text.isdigit(), state=SurxondaryoHomeSotishHovli.qavat)
async def check_umumiy(message: types.Message):
    await message.reply("❗ Фақат рақамда ёзинг")


@dp.message_handler(state=SurxondaryoHomeSotishHovli.qavat)
async def umumiymaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "qavat": text
...
# ====================================================================
@dp.callback_query_handler(text='Evroremont', state=SurxondaryoHomeSotishHovli.remont, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = "Евроремонт"
    await callback_query.answer("Pressed")
    await state.update_data({
...
@dp.callback_query_handler(text="Ta'mirlangan", state=SurxondaryoHomeSotishHovli.remont, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = "Таъмирланган"
    await callback_query.answer("Pressed")
    await state.update_data({
...
@dp.callback_query_handler(text="O'rtacha", state=SurxondaryoHomeSotishHovli.remont, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = "Ўртача"
    await callback_query.answer("Pressed")
    await state.update_data({
...
@dp.callback_query_handler(text="Ta'mirsiz", state=SurxondaryoHomeSotishHovli.remont, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = "Таъмирсиз"
    await callback_query.answer("Pressed")
    await state.update_data({
...
@dp.callback_query_handler(text='Mavjud', state=SurxondaryoHomeSotishHovli.jihozlar, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = "бор"
    await callback_query.answer("Pressed")
    await state.update_data({
...
@dp.callback_query_handler(text='Jihozlarsiz', state=SurxondaryoHomeSotishHovli.jihozlar, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = "йўқ"
    await callback_query.answer("Pressed")
    await state.update_data({
...
@dp.callback_query_handler(text="bor", state=SurxondaryoHomeSotishHovli.gaz)
...
@dp.callback_query_handler(text="yoq", state=SurxondaryoHomeSotishHovli.gaz)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext):
    text = "doesnotexist"
    await callback_query.answer("Танланди")
...
@dp.callback_query_handler(text="bor", state=SurxondaryoHomeSotishHovli.svet)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext):
    text = "Свет ✔️"
    await callback_query.answer("Танланди")
...
@dp.callback_query_handler(text="yoq", state=SurxondaryoHomeSotishHovli.svet)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext):
    text = "doesnotexist"
    await callback_query.answer("Танланди")
...
@dp.callback_query_handler(text="bor", state=SurxondaryoHomeSotishHovli.suv)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext):
    text = "Сув ✔️"
...
@dp.callback_query_handler(text="yoq", state=SurxondaryoHomeSotishHovli.suv)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext):
    text = "doesnotexist"
    await callback_query.answer("Tanlandi")
...
@dp.callback_query_handler(text="bor", state=SurxondaryoHomeSotishHovli.kanal)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext):
    text = "Канализация  ✔️"
    await callback_query.answer("Tanlandi")
...
@dp.callback_query_handler(text="yoq", state=SurxondaryoHomeSotishHovli.kanal)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext):
    text = "doesnotexist"
    await callback_query.answer("Tanlandi")
...
@dp.message_handler(state=SurxondaryoHomeSotishHovli.qoshimchaMalumot)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
...
@dp.callback_query_handler(text='USD', state=SurxondaryoHomeSotishHovli.valyuta, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = " $"
    await callback_query.answer("Pressed")
...
@dp.callback_query_handler(text='SUM', state=SurxondaryoHomeSotishHovli.valyuta, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = " сўм"
    await callback_query.answer("Pressed")
...
@dp.message_handler(lambda message: not message.text.isdigit(), state=SurxondaryoHomeSotishHovli.narxi)
async def check_umumiy(message: types.Message):
    await message.reply("❗ Фақат рақамда ёзинг")


@dp.message_handler(state=SurxondaryoHomeSotishHovli.narxi)
...
@dp.message_handler(state=SurxondaryoHomeSotishHovli.manzil)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "manzil": text
...
@dp.message_handler(state=SurxondaryoHomeSotishHovli.moljal)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "moljal": text
...
@dp.message_handler(state=SurxondaryoHomeSotishHovli.telNumberOne)
async def umumiyMaydon(message: types.Message, state: FSMContext):
...
@dp.message_handler(state=SurxondaryoHomeSotishHovli.telNumberTwo)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "telNumberTwo": text
...
@dp.message_handler(state=SurxondaryoHomeSotishHovli.check)
