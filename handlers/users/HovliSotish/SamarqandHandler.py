...
async def first(callback_query: types.CallbackQuery):
    ...
    await SamarqandHomeSotishHovli.umumiyMaydon.set()


@dp.message_handler(lambda message: not message.text.isdigit(), state=SamarqandHomeSotishHovli.umumiyMaydon)
...
@dp.callback_query_handler(text='yoq', state=SamarqandHomeSotishHovli.jihozlar, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = "йўқ"
    await callback_query.answer("Pressed")
    await state.update_data({
...
@dp.message_handler(is_media_group=True, state=SamarqandHomeSotishHovli.images, content_types=types.ContentTypes.PHOTO)
