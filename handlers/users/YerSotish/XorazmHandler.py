...
async def first(callback_query: types.CallbackQuery):
    ...
    await XorazmYerSotish.images.set()


@dp.message_handler(is_media_group=True, state=XorazmYerSotish.images, content_types=types.ContentTypes.ANY)
...
@dp.message_handler(lambda message: not message.text.replace('.', '').replace(',', '').isdigit(), state=XorazmYerSotish.umumiyMaydon)
