...
async def first(callback_query: types.CallbackQuery):
    ...
    await NavoiyYerSotish.images.set()


@dp.message_handler(is_media_group=True, state=NavoiyYerSotish.images, content_types=types.ContentTypes.ANY)
...
@dp.message_handler(lambda message: not message.text.replace('.', ',').isdigit(), state=NavoiyYerSotish.umumiyMaydon)
