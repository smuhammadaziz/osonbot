...
async def first(callback_query: types.CallbackQuery):
    ...
    await SamarqandYerSotish.images.set()


@dp.message_handler(is_media_group=True, state=SamarqandYerSotish.images, content_types=types.ContentTypes.ANY)
...
@dp.message_handler(lambda message: not message.text.replace('.', '').replace(',', '').isdigit(), state=SamarqandYerSotish.umumiyMaydon)
