...
@dp.callback_query_handler(text="andijonyer", state=None, chat_type="private")
async def first(callback_query: types.CallbackQuery):
    ...
    await AndijonYerSotish.umumiyMaydon.set()


@dp.message_handler(lambda message: not message.text.isdigit(), state=AndijonYerSotish.umumiyMaydon)
async def check_umumiy(message: types.Message):
    await message.reply("❗ Фақат рақамда ёзинг")


@dp.message_handler(state=AndijonYerSotish.umumiyMaydon)
async def umumiymaydon(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(chat_id=message.chat.id, text="<b> 🔥 Газ борми? </b>", parse_mode="HTML",
                           reply_markup=borYoq)
    await AndijonYerSotish.next()


# ================================================================

@dp.callback_query_handler(text="bor", state=AndijonYerSotish.gaz)
...
@dp.callback_query_handler(text="yoq", state=AndijonYerSotish.gaz)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext):
    text = "doesnotexist"
    await callback_query.answer("Танланди")
...
@dp.callback_query_handler(text="bor", state=AndijonYerSotish.svet)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext):
    text = "Свет ✔️"
    await callback_query.answer("Танланди")
...
@dp.callback_query_handler(text="yoq", state=AndijonYerSotish.svet)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext):
    text = "doesnotexist"
    await callback_query.answer("Танланди")
...
@dp.callback_query_handler(text="bor", state=AndijonYerSotish.suv)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext):
    text = "Сув ✔️"
...
@dp.callback_query_handler(text="yoq", state=AndijonYerSotish.suv)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext):
    text = "doesnotexist"
    await callback_query.answer("Tanlandi")
...
@dp.callback_query_handler(text="bor", state=AndijonYerSotish.kanal)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext):
    text = "Канализация ✔️"
    await callback_query.answer("Tanlandi")
...
@dp.callback_query_handler(text="yoq", state=AndijonYerSotish.kanal)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext):
    text = "doesnotexist"
    await callback_query.answer("Tanlandi")
...
@dp.message_handler(state=AndijonYerSotish.qoshimchaMalumot)
async def qoshimchamalumot(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
...
@dp.callback_query_handler(text='dokumentBor', state=AndijonYerSotish.hujjatlar, chat_type="private")
...
@dp.callback_query_handler(text='dokumentYoq', state=AndijonYerSotish.hujjatlar, chat_type="private")
async def dokumentlar(callback_query: types.CallbackQuery, state: FSMContext):
    text = " Тайёр эмас"
...
@dp.callback_query_handler(text='USD', state=AndijonYerSotish.valyuta, chat_type="private")
...
@dp.callback_query_handler(text='SUM', state=AndijonYerSotish.valyuta, chat_type="private")
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext):
    text = " сўм"
    await callback_query.answer("Pressed")
...
@dp.message_handler(lambda message: not message.text.isdigit(), state=AndijonYerSotish.narxi)
async def check_umumiy(message: types.Message):
    await message.reply("❗ Фақат рақамда ёзинг")


@dp.message_handler(state=AndijonYerSotish.narxi)
...
@dp.message_handler(state=AndijonYerSotish.manzil)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "manzil": text
...
@dp.message_handler(state=AndijonYerSotish.moljal)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "moljal": text
...
@dp.message_handler(state=AndijonYerSotish.telNumberOne)
async def umumiyMaydon(message: types.Message, state: FSMContext):
...
@dp.message_handler(state=AndijonYerSotish.telNumberTwo)
...
@dp.message_handler(is_media_group=True, state=AndijonYerSotish.images, content_types=types.ContentTypes.PHOTO)
...
@dp.message_handler(state=AndijonYerSotish.check)
